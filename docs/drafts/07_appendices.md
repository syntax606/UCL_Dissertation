...
The two auxiliary annotators in A.6 worked from separate packages that carried their own
...
**All figures in this section are single-partition**, predating the scheme in [3.7]. They
differ from [4.2] by up to 0.008 for that reason, so this section is the reference for the
...
in this section are single-partition, predating the scheme in [3.7], while [4.3] is a mean over 25
partitions. The gap is therefore a difference of measurement scheme rather than sampling noise, and
this section is retained for the ordering it shows across readouts rather than for its absolute
values.
