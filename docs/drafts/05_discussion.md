...
The natural reading of a codec losing pragmatic content is that it discards the acoustic detail that content is built from. That reading is wrong. Recovering hand-crafted acoustic cues from each rung, every codec rung does better than WavLM on every group except one, reaching 152 per cent on the contour features that stance is built from and 162 per cent on voice quality, on half the feature dimensions WavLM uses [4.4].
...
Two independent measurements say the missing organisation is temporal. Temporal is the one cue group the codecs fail to retain, falling to 78 per cent for the deployed histogram and 61 to 67 per cent for DAC while no other group falls below 97 per cent anywhere on the codec ladder. And the order effect, measured against a frame-shuffled control at matched dimensionality, declines along the same ladder. Those two measurements share no machinery, so their agreement is not an artefact of either.
...
Variable-frame-rate tokenisation does not recover the loss. Measured against each configuration's own null, Mimi before quantisation reaches +0.137 against Sylber's +0.111 and DyCAST's +0.076 [4.6]. DyCAST is clearly below and Sylber is level, since 0.026 is inside the partition-noise threshold. Neither exceeds Mimi. The cue analysis says why rather than only that. DyCAST retains 44 per cent of the temporal group, the least of any representation measured, and Sylber sits below WavLM on every group [4.4]. Abandoning the clock does not by itself preserve the organisation the clock was carrying. This is consistent with what those systems set out to do rather than a failure of them. The syllabic lineage Sylber belongs to is built to extract coarse semantic units at low bitrate, and observes that existing tokenisations predominantly capture phonetic information (Baade et al., 2025). DyCAST aligns to characters, which is again a linguistic unit. A literature proposing dynamic frame rates to preserve timing (FlexiCodec and the systems following it) is therefore proposing the right diagnosis with a mechanism these results suggest is insufficient [6.3].
...
The order-aware summaries lose to counting [4.6]. Run lengths and change rates are meaningful operations on categorical codes, and the plain histogram beats them anyway. Taken with the instability Liu et al. describe, the reading is that code identity is too unstable a quantity to support temporal statistics over it.
