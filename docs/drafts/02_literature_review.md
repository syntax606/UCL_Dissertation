...
The acoustic signature. An experimental literature converges on a small set of correlates for ironic delivery, namely slower tempo, greater intensity, lowered pitch, reduced F0 range and altered voice quality (Rockwell, 2000; Lan et al., 2019), while disputing whether these amount to a dedicated ironic tone or a family of cues recognised in context (Bryant and Fox Tree, 2005). That dispute bears on the premise check in [4.1], where context alone recovers much of the contrast. Two of those five correlates describe movement rather than position, since tempo is a rate and a reduced range is a narrowed excursion, which is the distinction [4.4] measures on this corpus. One tension is worth naming. Rockwell leads with tempo, and coarse rate is the cue group carrying least stance here, at +0.024 against contour's +0.083. The measures are not the same quantity, since hers is a perceptual coding of filtered speech and this is a segment-rate statistic, but the emphasis differs and the difference is not resolved here. Appendix [C] gives the inventory.
